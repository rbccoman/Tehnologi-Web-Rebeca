...
/**
 * EXPORT: GET /
 * Returns: [{...university, students:[], courses:[], enrollements:[{studentId, courseId}]}]
 */
   ...
   const universities = await University.findAll({
    ...
    if (!universities || universities.length === 0) {
     ...
     const [rows] = await sequelize.query(
      'SELECT studentId, courseId FROM enrollements;'
    );

    const result = universities.map((u) => {
      const uni = u.toJSON();

      const studentIds = new Set((uni.students || []).map((s) => s.id));
      const courseIds = new Set((uni.courses || []).map((c) => c.id));

      const enrollements = (rows || []).filter(
